...
class Helper_Shift_Irregular{
  ...
  // 当該イレギュラーシフトグループの特定ユーザの確定勤務日数を求める(PDF用)
  public static function deside_work_day_count_pdf($irregular_id, $user_id){
    $irregular_day_id = DB::select('id')->from('irregular_day')->where('irregular_id', $irregular_id);
    $work_day = DB::select('*')->from('irregular_user')
        ->where('user_id', $user_id)
        ->and_where('edited_shift_type', 'in', array(1,2,3))
  ...
  // 当該イレギュラーシフトグループの特定ユーザの確定勤務時間を求める(pdf用)
  public static function deside_work_time_count_pdf($irregular_id, $user_id){
    $irregular_day_id = DB::select('id')->from('irregular_day')->where('irregular_id', $irregular_id);
    $morning = DB::select('*')->from('irregular_user')
        ->where('user_id', $user_id)
        ->and_where('edited_shift_type', 1)
    ...
    $full = DB::select('*')->from('irregular_user')
        ...
        ->and_where('irregular_day_id', 'in', $irregular_day_id)
        ->execute();
    return count($morning)*3 + count($afternoon) * 4 + count($full) * 6;
  }
  ...
  // 当該イレギュラーシフトグループの確定勤務時間の合計を求める(PDF用)
  public static function deside_irregular_group_total_work_time_pdf($irregular_id){
    $irregular_day_id = DB::select('id')->from('irregular_day')->where('irregular_id', $irregular_id);
    $morning = DB::select('*')
        ->from('irregular_user')
        ->where('irregular_day_id', 'in', $irregular_day_id)
